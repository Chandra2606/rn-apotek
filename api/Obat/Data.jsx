...
export default function Data() {
  ...
  const onRefresh = () => {
    setRefreshing(true);
    fetchDataObat(1, search).finally(() => setRefreshing(false));
  };
  ...
  const rederItemObat = ({item}) => {
    const showDetailData = () => {
      navigation.navigate('DetailObat', {kodeobat: item.kodeobat});
    };
    return (
      <TouchableOpacity style={styles.item} onPress={showDetailData}>
        <Text style={styles.title}>{item.namaobat}</Text>
        <Text style={styles.nim}>{item.kodeobat}</Text>
      </TouchableOpacity>
    );
  };
