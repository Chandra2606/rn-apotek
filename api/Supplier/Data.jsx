...
export default function Data() {
  ...
  //memanggil api
  const fetchDataSupplier = async (pageNumber = 1, searchQuery = search) => {
    ...
    try {
      ...
      setDataSupplier(
        pageNumber === 1 ? json.data : [...dataSupplier, ...json.data],
      );
    } catch (error) {
  ...
  const onRefresh = () => {
    setRefreshing(true);
    fetchDataSupplier(1, search).finally(() => setRefreshing(false));
  };
  ...
  const rederItemSupplier = ({item}) => {
    const showDetailData = () => {
      navigation.navigate('DetailSupplier', {kodesup: item.kodesup});
    };
    return (
      <TouchableOpacity style={styles.item} onPress={showDetailData}>
        <Text style={styles.title}>{item.namasup}</Text>
        <Text style={styles.nim}>{item.kodesup}</Text>
      </TouchableOpacity>
    );
  };
